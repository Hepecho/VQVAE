...
class PixelCNN(nn.Module):
    """
    PixelCNN模型会根据前i - 1个像素输出第i个像素的概率分布
    """
...
class PixelCNNWithEmbedding(GatedPixelCNN):
    """
    PixelCNN with embedding是在PixelCNN前加了一个embedding层，因为对于VQ-VAE模型，它的latent code本质上是一串离散的编码
    而PixelCNN只能处理连续的像素分布预测，所以这里先用一个embedding映射，embedding层的color_level和vq_embedding的n_e相同1
    """
    def __init__(self, n_blocks, p, linear_dim, bn=True, color_level=256):
